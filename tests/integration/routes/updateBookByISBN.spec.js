...
describe("PATCH '/books/:id' route", () => {
  test("should update a book in the database", async () => {
    ...
    const response = await request(app)
      .patch(`/books/${bookFixtures[0].ISBN}`)
      .send(bookUpdate);
    ...
    expect(parseInt(response.body.updatedBook.publicationYear)).toBe(
      bookUpdate.publicationYear,
    );
  });
  ...
  test("should return an 404 if the book to be updated doesn't exist", async () => {
    const bookUpdate = bookFixtures[1];
    const response = await request(app)
      .patch(`/books/v-11ery-iinvalid-23-one`)
      .send(bookUpdate);
