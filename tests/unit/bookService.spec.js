...
describe("Book Service Tests", () => {
    ...
    it("should get a book by ISBN from the database", async () => {
        ...
        const book = await bookService.getBookByISBN(bookFixtures[0].ISBN, redisClient)
        ...
        expect(parseInt(book.publicationYear)).toBe(bookFixtures[0].publicationYear)
    })
    ...
    it("should update a book an existing book successfully", async () => {
        ...
        const book = await testHelper.getBookByISBN(bookFixtures[0].ISBN, redisClient)
        expect(book.ISBN).toBe(bookFixtures[0].ISBN)
    ...
    it("should delete an existing book successfully", async () => {
        await testHelper.createBook(bookFixtures[0], redisClient)
        const bookBeforeDelete = await testHelper.getBookByISBN(bookFixtures[0].ISBN, redisClient)
        await bookService.deleteBookByISBN(bookBeforeDelete.ISBN)
        const bookAfterDelete = await testHelper.getBookByISBN(bookFixtures[0], redisClient)
        ...
        expect(parseInt(bookBeforeDelete.publicationYear)).toBe(bookFixtures[0].publicationYear)
